...
secrets:
  - name: LLM_API_KEY
    description: "Your LLM provider API key for direct providers such as OpenRouter, Anthropic, OpenAI, Google, DeepSeek, xAI, Z.ai, and others."
  - name: LLM_MODEL
  ...
  - name: CUSTOM_BASE_URL
    description: "Optional custom OpenAI-compatible endpoint base URL (e.g. opencode.ai/zen/v1)."
  - name: CUSTOM_API_KEY
...
- 💬 **Discord Native:** Connect Hermes to Discord with a single bot-token secret — no shell access required.
- 🔌 **Custom Endpoints:** Point Hermes at any OpenAI-compatible `/v1` gateway, including the OpenCode Zen gateway.
- ⏰ **Wake-on-Demand:** An external, low-frequency trigger wakes a sleeping Space on demand and notifies you when it is ready.
- 🔄 **Self-Healing Gateway:** Gateway, dashboard, and health server are monitored and automatically restarted if they exit unexpectedly.
- 📦 **Ephemeral Package Replay:** Install packages via startup variables and they survive restarts — `apt`/`pip`/`uv`/`npm`/`hermes` installs are replayed on every boot.
...
- `DISCORD_ALLOWED_USERS` - Comma-separated numeric Discord user IDs.
- `HF_TOKEN` - Hugging Face token with write access for backups.

For a custom model gateway instead of a built-in provider:

- `CUSTOM_BASE_URL` - e.g. `https://opencode.ai/zen/v1`
- `CUSTOM_API_KEY` - your key for that endpoint
- `LLM_MODEL` - the model ID to use, e.g. `deepseek-v4-flash-free`
...
To use Hermes via Discord:

1. Create a bot application in the [Discord Developer Portal](https://discord.com/developers/applications).
2. Copy the bot token and add it as the `DISCORD_BOT_TOKEN` secret in your Space.
3. (Optional) Restrict access with `DISCORD_ALLOWED_USERS` — comma-separated numeric user IDs.
4. Invite the bot to your server with the appropriate permissions (read messages, send messages).

That's it — Hermes discovers the Discord platform automatically at boot. No shell access or interactive setup is required; all configuration is environment-driven.

## 🔌 Custom Endpoints

Common setup for a custom OpenAI-compatible gateway (e.g. the OpenCode Zen gateway at `https://opencode.ai/zen/v1`):

| Variable | Value |
| :--- | :--- |
| `CUSTOM_BASE_URL` | `https://opencode.ai/zen/v1` |
| `CUSTOM_API_KEY` | your key (a placeholder is acceptable where the gateway does not require one) |
| `LLM_MODEL` | model ID, e.g. `deepseek-v4-flash-free` |
| `CUSTOM_MODEL_CONTEXT_LENGTH` | context window of the configured model |
| `CUSTOM_MODEL_MAX_TOKENS` | max output tokens |

For advanced per-provider settings (custom request headers, TLS options), create a `providers.<name>` entry in `config.yaml` — Hermes matches it by `base_url` and applies the extra headers automatically.
...
Install packages with startup variables and they survive Space restarts — shell wrappers record every successful `apt install`, `pip install`, `uv pip install`, `npm install -g`, and `hermes plugins install` into `workspace/startup.sh`, which is backed up and replayed automatically on next boot.
...
> Note: `HUGGINGMES_APT_PACKAGES` requires root privileges inside the container. HF Spaces run containers as a non-root user, so prefer `pip`/`npm` packages unless your Space runs as root.
...
Free HF Spaces sleep after a period of inactivity. Instead of any fixed-interval pinging, this project uses an **on-demand wake** pattern:

1. A GitHub Actions workflow (or any scheduler) fires a single HTTPS GET to your Space's root URL — once, on demand, no more often than once per day.
2. Optionally, a Discord→GitHub webhook (a small external relay) triggers the same workflow instantly when you type a wake command in Discord.
3. When the Space finishes booting (1–2 minutes cold start), the `WEBHOOK_URL` endpoint receives a JSON "started" notification — so you know it is awake before you interact.

No aggressive pings, no scheduled scans — just a single request when you actually need the agent.
...
| `CUSTOM_BASE_URL` | — | Custom OpenAI-compatible endpoint base URL |
| `CUSTOM_API_KEY` | — | Key for the custom endpoint |
| `CUSTOM_MODEL_CONTEXT_LENGTH` | `131072` | Context window in tokens for the configured model |
| `CUSTOM_MODEL_MAX_TOKENS` | `8192` | Max output tokens |
| `SYNC_INTERVAL` | `600` | Backup frequency in seconds |
| `WEBHOOK_URL` | — | Endpoint for POST JSON start notifications |
...
Provider API keys and tokens are read from the environment at boot — set them as **Space Secrets**, never via the dashboard's env editor. Secrets stored in the Hermes `config.yaml` are redacted before backup sync (see `hermes-sync.py`).

## 🏗️ Architecture

- **Dashboard (`/`)**: Real-time management and monitoring.
- **Hermes App (`/app/`)**: Secure forwarded access to the Hermes UI.
...
Spaces can reach most public APIs directly. Endpoints that HF blocks from Spaces (e.g. the Telegram API) are simply out of scope for this deployment — the Discord platform is used instead and is reachable without any workaround. Before relying on any third-party endpoint (including the model gateway), verify it is reachable from inside a Space with a direct request; see `DEPLOYMENT_README.md`.

## 🐛 Troubleshooting

- **Discord bot not responding:** Verify `DISCORD_BOT_TOKEN` is set as a Space Secret, the bot is invited to the server, and your user ID is in `DISCORD_ALLOWED_USERS` if you set it. Check `$HERMES_HOME/logs/gateway.log`.
...
- **Space keeps sleeping:** Use the wake-on-demand trigger described above — a single request when you need the agent, then wait for the start notification.
